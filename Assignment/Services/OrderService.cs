...
namespace Assignment
{
	public class OrderService
	{
		...
		public void ModifyOrderDetails()
		{
			...
			Customer selectedCustomer = SelectCustomerFromListWithOrders(customersWithOrders);
		...
		private Customer SelectCustomerFromListWithOrders(List<Customer> customers)
		{
			while (true)
			{
				try
		...
		private void ModifyOrder(Order order)
		{
			...
			Console.WriteLine("\nChoose an action:");
			Console.WriteLine("1. Modify an Ice Cream");
		...
		private void ModifyExistingIceCream(Order order)
		{
			if (!order.IceCreamList.Any())
			{
				Console.WriteLine("No ice creams to modify.");
				return;
			}

			int index = GetValidIceCreamIndex(order, "modify");
			if (index == -1) return;
			...
			while (true)
			{
				try
				{
					...
					{
						case 1:
							ModifyIceCreamOption(order, index);
							return;
						case 2:
							ModifyIceCreamScoops(order, index);
							return;
						case 3:
							ModifyIceCreamFlavours(order, index);
							return;
						case 4:
							ModifyIceCreamToppings(order, index);
							return;
						default:
		...
		private void ModifyIceCreamOption(Order order, int index)
		{
			IceCream currentIceCream = order.IceCreamList[index];
			string newOption = GetValidIceCreamOption();

			IceCream newIceCream = CreateSpecificIceCream(newOption, currentIceCream.Scoops,
				currentIceCream.Flavours, currentIceCream.Toppings);

			if (newIceCream != null)
			{
				order.IceCreamList[index] = newIceCream;
				Console.WriteLine("Ice cream option updated successfully!");
			}
		}

		private void ModifyIceCreamScoops(Order order, int index)
		{
			int newScoops = GetValidScoopCount();
			order.IceCreamList[index].Scoops = newScoops;
			Console.WriteLine("Number of scoops updated successfully!");
		}

		private void ModifyIceCreamFlavours(Order order, int index)
		{
			IceCream iceCream = order.IceCreamList[index];

			Console.WriteLine("Current flavours:");
			for (int i = 0; i < iceCream.Flavours.Count; i++)
			{
				Console.WriteLine($"{i + 1}. {iceCream.Flavours[i].Type}");
			}

			if (GetYesNoAnswer("Do you want to replace all flavours? (Y/N): "))
			{
				iceCream.Flavours = SelectFlavours(iceCream.Scoops);
			}
			else
			{
				// Add or remove individual flavours
				if (GetYesNoAnswer("Add a new flavour? (Y/N): "))
				{
					var newFlavours = SelectFlavours(1);
					iceCream.Flavours.AddRange(newFlavours);
				}

				if (iceCream.Flavours.Count > 1 && GetYesNoAnswer("Remove a flavour? (Y/N): "))
				{
					int flavourIndex = GetValidIceCreamIndex(order, "remove") - 1;
					if (flavourIndex >= 0 && flavourIndex < iceCream.Flavours.Count)
					{
						iceCream.Flavours.RemoveAt(flavourIndex);
					}
				}
			}
		...
		private void ModifyIceCreamToppings(Order order, int index)
		{
			IceCream iceCream = order.IceCreamList[index];
			...
			{
				Console.WriteLine($"{i + 1}. {iceCream.Toppings[i].Type}");
			}

			if (GetYesNoAnswer("Do you want to replace all toppings? (Y/N): "))
			{
				iceCream.Toppings = SelectToppings();
			}
			else
			{
				if (GetYesNoAnswer("Add new toppings? (Y/N): "))
				{
					var newToppings = SelectToppings();
					iceCream.Toppings.AddRange(newToppings);
				}

				if (iceCream.Toppings.Any() && GetYesNoAnswer("Remove a topping? (Y/N): "))
				{
					Console.WriteLine("Select topping to remove:");
					for (int i = 0; i < iceCream.Toppings.Count; i++)
					{
						Console.WriteLine($"{i + 1}. {iceCream.Toppings[i].Type}");
					}

					int toppingIndex = GetValidToppingIndex(iceCream.Toppings);
					if (toppingIndex != -1)
					{
						iceCream.Toppings.RemoveAt(toppingIndex);
					}
				}
			}
		...
		private int GetValidToppingIndex(List<Topping> toppings)
		{
			while (true)
			{
				try
				{
					Console.Write("Enter topping number to remove: ");
					int index = Convert.ToInt32(Console.ReadLine()) - 1;
